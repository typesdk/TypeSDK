...
//	private ProgressDialog mProgress;
	...
	/** 角色信息采集接口  */ 
	protected void doSdkGetUserInfoByCP(String type) {      
		HashMap eventParams=new HashMap();     
		//----------------------------参数设置说明-------------------- 
		//以下列出的五项只是作为参考，请按照上述表格中的参数说明进行补充添加 
		//仅按 demo 样例上传无法通过审核！请务必仔细阅读上述表格！         
		eventParams.put("zoneid",userInfo.GetInt(AttName.SERVER_ID));//当前角色所在游戏区服 id         
		eventParams.put("zonename",userInfo.GetData(AttName.SERVER_NAME));//当前角色所在游戏区服名称         
		eventParams.put("roleid",userInfo.GetData(AttName.ROLE_ID));//当前角色 id         
		eventParams.put("rolename",userInfo.GetData(AttName.ROLE_NAME));//当前角色名称 
		...
		eventParams.put("rolelevel",userInfo.GetInt(AttName.ROLE_LEVEL));
		...
		mainHandler.post(new Runnable() {
			...
			public void run() {
				try {
					...
					Matrix.setActivity(appActivity,mSDKCallback);
				} catch (Exception e) {
					e.printStackTrace();
...
//	    	mGetUserInfoFlag = !mGetUserInfoFlag;
	...
	public void getAge(){
       ...
       // 必需参数，用户access token，要使用注意过期和刷新问题，最大64字符。
        ...
        // 必需参数，360帐号id。
        bundle.putString(ProtocolKeys.QIHOO_USER_ID, userInfo.GetData(AttName.USER_ID));
        ...
        Matrix.execute(appContext, intent, new IDispatcherCallback() {
            @Override
            public void onFinished(String data) {
				try {
					JSONObject json = new JSONObject(data);
					if(json.getString("error_code").equals("0")){
						json = new JSONObject(json.getString("content"));
						JSONArray retJsonarr = new JSONArray(json.getString("ret"));
						String[] ret = new String[retJsonarr.length()];
						for (int i =  0; i < retJsonarr.length(); i++) {
							JSONObject temp = (JSONObject) retJsonarr.get(i);
							ret[i] = temp.getString("status");
						}
						String age = ret[0];
