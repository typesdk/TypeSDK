...
//					TypeSDKLogger.e("APP_ID:" + platform.GetData(AttName.APP_ID));
					GamePlatform.requestFloatWindowsPermission(appActivity);
...
//		setSuspendWindowChangeAccountListener();
	...
	private void amigoPay(PayInfoData _in_pay) {
		...
		mGamePayCallback = new GamePayCallBack() {
            ...
            @Override
            public void onPayFail(Exception stateCode) {
                // 可以在这里处理自己的业务
            	payResult.SetData(AttName.PAY_RESULT, "0");
				payResult.SetData(AttName.PAY_RESULT_REASON, "PAY_FAIL");
				notify.Pay(payResult.DataToString());
...
//	        		        TypeSDKLogger.v("SubmitTime:" + new SimpleDateFormat("yyyyMMddHHmmss").format(new Date()));
	        		        TypeSDKLogger.d("orderInfo:" + orderInfo);
	        		      //调用启动收银台接�? 
	        		        new Handler(Looper.getMainLooper()).post(new Runnable() {
	        					@Override
	        					public void run() {
	        						try {
	        							mGamePayManager.pay(appActivity, orderInfo, mGamePayCallback);
	        						} catch (Exception e) {
	        							// TODO Auto-generated catch block
	        							e.printStackTrace();
	        						}
