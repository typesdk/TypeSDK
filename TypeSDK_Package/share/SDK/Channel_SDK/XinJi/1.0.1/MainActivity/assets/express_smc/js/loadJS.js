...
function loadScript(path,callback) {
	expressSmcApp.testlog("load_r_express_smc,path="+path);
	if(typeof collectTraceInfo =='function'){
    	collectTraceInfo("loadScript,path="+path);
    }
	begin_Time=Date.now();
	var head = document.getElementsByTagName('head')[0];
//	var pageTitle=head.getElementsByTagName('title')[0];
...
var load_css=false,load_express_smc=false,load_ex_control=false,load_ex_data=false,load_ex_page=false,load_global=false,load_iscroll=false,load_select=false,load_m=false,load_control=false,load_common=false,load_page=false,load_data=false;
var load_css=false,load_AllJS=false;
var isComplete=false;//是否加载main方法
function loadJsAndCssComplete(jsAndCssName){
	//var isLoaded=load_express_smc&&load_ex_control&&load_ex_data&&load_ex_page&&load_css&&load_global&&load_iscroll&&load_select&&load_m&&load_control&&load_common&&load_page&&load_data;
	...
	if(isLoaded){
		expressSmcApp.testlog("loadJsAndCssComplete(),isLoaded");
		main(htmlVersion);//加载完成,执行页面渲染
		isComplete=true;//加载和渲染完成
		expressSmcApp.loadWebViewFinish();
		if(typeof collectTraceInfo =='function'){
...
function loadingJsAndCss(){
	expressSmcApp.testlog("loadingJsAndCss()");
	//同步加载CSS和JS
	...
	loadScript(baseUrl+"js/r_express_smc.js?v="+jsVersion,function(){//再同步加载JS
		load_AllJS=true;
        loadJsAndCssComplete('r_express_smc.js');
        expressSmcApp.testlog("load_r_express_smc=true");
	});

	/*loadScript(baseUrl+"js/s.R.c/global.js?v="+jsVersion,function(){//再同步加载JS
		load_global=true;
		expressSmcApp.testLog("load_global=true");
	...
	loadScript(baseUrl+"js/s.R.c/iscroll.js?v="+jsVersion,function(){//再同步加载JS
		...
		expressSmcApp.testlog("load_iscroll=true");
	});
	loadScript(baseUrl+"js/s.R.c/select.js?v="+jsVersion,function(){//再同步加载JS
		load_select=true;
		loadJsAndCssComplete('select.js');
		expressSmcApp.testlog("load_select=true");
	});
	loadScript(baseUrl+"js/s.R.c/m.js?v="+jsVersion,function(){//再同步加载JS
		load_m=true;
        loadJsAndCssComplete('m.js');
        expressSmcApp.testlog("load_m=true");
	});
	loadScript(baseUrl+"js/s.R.c/control.js?v="+jsVersion,function(){//再同步加载JS
		load_control=true;
        loadJsAndCssComplete('control.js');
        expressSmcApp.testlog("load_control=true");
	});
	loadScript(baseUrl+"js/s.R.c/common.js?v="+jsVersion,function(){//再同步加载JS
		load_common=true;
        loadJsAndCssComplete('common.js');
        expressSmcApp.testlog("load_common=true");
	});
	loadScript(baseUrl+"js/s.R.c/page.js?v="+jsVersion,function(){//再同步加载JS
		load_page=true;
        loadJsAndCssComplete('page.js');
        expressSmcApp.testlog("load_page=true");
	});
	loadScript(baseUrl+"js/s.R.c/data.js?v="+jsVersion,function(){//再同步加载JS
		load_data=true;
        loadJsAndCssComplete('data.js');
        expressSmcApp.testlog("load_data=true");
	});
	loadScript(baseUrl+"js/s.R.c/express_smc.js?v="+jsVersion,function(){//再同步加载JS
		load_express_smc=true;
        loadJsAndCssComplete('express_smc.js');
        expressSmcApp.testlog("load_express_smc=true");
	});
	
	
	loadScript(baseUrl+"js/s.R.c/express/ex_control.js?v="+jsVersion,function(){//再同步加载JS
		load_ex_control=true;
        loadJsAndCssComplete('ex_control.js');
        expressSmcApp.testlog("load_ex_control=true");
	});
	loadScript(baseUrl+"js/s.R.c/express/ex_data.js?v="+jsVersion,function(){//再同步加载JS
		load_ex_data=true;
        loadJsAndCssComplete('ex_data.js');
        expressSmcApp.testlog("load_ex_data=true");
	});
	loadScript(baseUrl+"js/s.R.c/express/ex_page.js?v="+jsVersion,function(){//再同步加载JS
		load_ex_page=true;
        loadJsAndCssComplete('ex_page.js');
        expressSmcApp.testlog("load_ex_page=true");
	});*/
...
function loadingVersion(){
	expressSmcApp.testlog("loadingVersion()");
	//取资源版本	
	try {
		if(typeof collectTraceInfo =='function'){
		...
		xhr.onreadystatechange=function(){
			expressSmcApp.testlog("xhr.responseText="+xhr.responseText);
			expressSmcApp.testlog("onreadystatechange,xhr.readyState="+xhr.readyState+",xhr.status="+xhr.status);
			if(xhr.readyState == 4){
				if(xhr.status >= 200 && xhr.status <300 || xhr.status== 304){
					...
					expressSmcApp.testlog("jsVersion="+jsVersion+",htmlVersion="+htmlVersion+",cssVersion="+cssVersion);
					if(typeof collectTraceInfo =='function'){
