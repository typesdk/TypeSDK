...
function convertParamLogin(query,ret)
{
    ...
    ret.state = cloned.token;
    ret.uid = cloned.id;
    return true;
}
function creatsign(attrs,paramArr,queryObj){
    var str = '';

    for(var i=0;i<paramArr.length;i++){
        if(queryObj[paramArr[i]]){
            str+=queryObj[paramArr[i]];
        }else{
            str+=attrs[paramArr[i]];
        }
    }
...
function callChannelLogin(attrs,params,query,ret,retf)
{
    ...
    var signArray =['appid','uid','state','product_key'];
    cloned.flag = creatsign(attrs,signArray,cloned);
    var options = {
        url: params.out_url,
...
function checkSignPay(attrs,query)
{
    var sign = query.flag;
    delete query.flag;
    var signArray=['uid','money','time','sid','orderid','ext','pay_key'];

    var osign = creatsign(attrs,signArray,query);
...
function checkChOrder(game, channel,attrs, query, retf){
    ...
    logicCommon.selCHOrderInRedis(channel,query.orderid,function(res){
        if(!res || typeof res == "undefined"){
            logicCommon.saveCHOrderInRedis(game, channel, query.ext, query.orderid,function(res){
                if(res && typeof res != "undefined"){
                    isIllegal = true;
                    retf(isIllegal);
                }else{
