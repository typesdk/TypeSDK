...
{
    var retValue = {};
    retValue.code = query.statusCode  == '0000' ? 0 : 1; //success 字段未定
    retValue.id = query.productId;
    retValue.order = query.transactionNo;
    retValue.cporder = query.partnerTransactionNo || '';
    retValue.info = '';
    ...
    logicCommon.getNotifyUrl(retValue.cporder,params,function(hasData) {
        if (!hasData) {
            ...
            retValue.amount = '' + (query.orderPrice)*100 + '';
...
function checkSignPay(attrs,query)
{
    ...
    var keys2 = Object.keys(cloned);
    keys2 = keys2.sort();
    var str ='';
    for(var i=0;i<keys2.length;i++){
        str += keys2[i]+'='+cloned[keys2[i]]+'&';
    }
    str = str+attrs.secret_key;
    console.log(str);
...
function checkChOrder(game, channel,attrs, query, retf){
    var isIllegal = false;
    logicCommon.selCHOrderInRedis(channel,query.transactionNo,function(res){
        if(!res || typeof res == "undefined"){
            logicCommon.saveCHOrderInRedis(game, channel, query.partnerTransactionNo || '', query.transactionNo,function(res){
                if(res && typeof res != "undefined"){
                    isIllegal = true;
                    retf(isIllegal);
                }else{
