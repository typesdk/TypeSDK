...
function callChannelLogin(attrs,params,query,ret,retf)
{
    ...
    merge(cloned,query);
    cloned.game_id = attrs.app_key;
    cloned.session =  query.session;
    cloned.user_id =  query.user_id;
    var Str = "game_id=" + cloned.game_id + "&" +
        "session=" + cloned.session + "&" +
        "user_id=" +cloned.user_id;
    var sign = crypto.createHash('md5').update(Str).digest('hex');
    cloned._sign =  crypto.createHash('md5').update(sign + attrs.secret_key).digest('hex');
    var options = {
        url: params.out_url,
...
function compareOrder(attrs,gattrs,params,query,ret,game,channel,retf)
{
    ...
    retValue.order = query.transaction;
    retValue.cporder =  query.gameExtend;
    retValue.info = "";
                    ...
                    if(retOut.code=='0'){
                        ...
                        if(query.money*100>=retOut.amount*0.9
                            ...
                            if(retOut.status=='4'||retOut.status=='3'){
                                ...
                                var retDate = {};
                                retDate.status = 0;
                                retDate.transIDO = query.transaction;
                                retf(JSON.stringify(retDate));
                                return;
                            }else{
...
function checkSignPay(attrs,query)
{
    var str = "amount=" + query.amount + "&" +
        "cardPoint=" + query.cardPoint + "&" +
        "currency=" + query.currency + "&" +
        "gameExtend=" + query.gameExtend + "&" +
        "gameUserId=" + query.gameUserId + "&" +
        "payType=" + query.payType + "&" +
        "platform=" + query.platform + "&" +
        "serverNo=" + query.serverNo + "&" +
        "status=" + query.status + "&" +
        "transaction=" + query.transaction + "&" +
        "transactionTime=" + query.transactionTime + "&" +
        "userId=" + query.userId;
    var md5str = crypto.createHash('md5').update(str).digest('hex');
    var osign = crypto.createHash('md5').update(md5str + attrs.secret_key).digest('hex');
    console.log(query._sign + " :: " + osign);
    if (query._sign != osign)
    {
        return false;
    }
    return true;
...
function checkChOrder(game, channel,attrs, query, retf){
    var isIllegal = false;
    logicCommon.selCHOrderInRedis(channel,query.transaction,function(res){
        if(!res || typeof res == "undefined"){
            logicCommon.saveCHOrderInRedis(game, channel, query.gameExtend, query.transaction,function(res){
                if(res && typeof res != "undefined"){
                    isIllegal = true;
                    retf(isIllegal);
                }else{
