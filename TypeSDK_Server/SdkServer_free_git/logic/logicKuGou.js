...
function compareOrder(attrs,gattrs,params,query,ret,game,channel,retf){
    ...
    retValue.cporder =  query.outorderid;
    retValue.info = "";
        ...
        if (!hasData) {
            ...
            request(options, function (error, response, body) {
                if(!error && response.statusCode == 200){
                    ...
                    if(query.outorderid == retOut.cporder
...
function checkChOrder(game, channel,attrs, query, retf){
    ...
    logicCommon.selCHOrderInRedis(channel,query.orderid,function(res){
        if(!res || typeof res == "undefined"){
            logicCommon.saveCHOrderInRedis(game, channel, query.outorderid, query.orderid,function(res){
                if(res && typeof res != "undefined"){
                    isIllegal = true;
                    retf(isIllegal);
                }else{
