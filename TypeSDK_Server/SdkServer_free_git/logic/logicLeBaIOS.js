...
function compareOrder(attrs,gattrs,params,query,ret,game,channel,retf){
    var retValue = {};
    retValue.code = query.t_status  == '1' ? 0 : 1;
    retValue.id = query.u_param;
    retValue.order = query.o_orderid;
    retValue.cporder = query.o_id || '';
    retValue.info = '';
    if(retValue.code!='0'){
        retf('FAILURE');
        return;
            ...
            request(options, function (error, response, body) {
                if(!error && response.statusCode == 200){
                    ...
                    if(retOut.code=='0'){
                        ...
                        if(query.o_id==retOut.cporder
                            &&parseInt(query.t_fee)*100>=retOut.amount*0.9
                            &&parseInt(query.t_fee)*100<=retOut.amount){
                                if(retOut.status=='2'){
                                    retf('FAILURE');
                                    return;
                                }else if(retOut.status=='4'||retOut.status=='3'){
                                    logicCommon.UpdateOrderStatus(game,channel,retValue.cporder,retValue.order,4,parseInt(query.t_fee)*100);
                                    retf('success');
                                    return;
                                }else{
...
function callGamePay(attrs,gattrs,params,query,ret,retf,game,channel,channelId)
{
    var retValue = {};
    retValue.code = query.t_status  == '1' ? 0 : 1;
    retValue.id = query.u_param;//还未确定
    retValue.order = query.o_orderid;
    retValue.cporder = query.o_id || '';
    retValue.info = '';
    if(retValue.code!='0'){
    ...
    logicCommon.getNotifyUrl(retValue.cporder,params,function(hasData) {
        if (!hasData) {
            ...
            retValue.amount = '' + parseInt(query.t_fee)*100 + '';
            var options = {
                url: params.out_url,
                method: params.method,
                body: retValue,
...
{
    var str = 'n_time='+encodeURIComponent(query.n_time)+'&'
        +'appid='+encodeURIComponent(query.appid)+'&'
        +'o_id='+encodeURIComponent(query.o_id)+'&'
        +'t_fee='+encodeURIComponent(query.t_fee)+'&'
        +'g_name='+encodeURIComponent(query.g_name)+'&'
        +'g_body='+encodeURIComponent(query.g_body)+'&'
        +'t_status='+encodeURIComponent(query.t_status);
    var osign = crypto.createHash('md5').update(str.replace(/\%20/g, '+')+attrs.app_key).digest('hex');
    console.log(query.o_sign + " :: " + osign);
    if (query.o_sign != osign)
    {
        return false;
    }
    return true;
...
function checkChOrder(game, channel,attrs, query, retf){
    var isIllegal = false;
    logicCommon.selCHOrderInRedis(channel,query.o_orderid,function(res){
        if(!res || typeof res == "undefined"){
            logicCommon.saveCHOrderInRedis(game, channel, query.o_id || '', query.o_orderid,function(res){
                if(res && typeof res != "undefined"){
                    isIllegal = true;
                    retf(isIllegal);
                }else{
