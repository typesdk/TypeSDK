...
function compareOrder(attrs,gattrs,params,query,ret,game,channel,retf){
    var retValue = {};
    retValue.code = query.trade_result  == 'TRADE_SUCCESS' ? 0 : 1;
    retValue.id = query.extra_info;
    retValue.order = query.out_trade_no;
    retValue.cporder = query.cooperator_order_no;
    retValue.info = '';
    ...
    logicCommon.getNotifyUrl(retValue.cporder,params,function(hasData) {
        ...
        else  if (query.app_order_id == params.orderdata && query.product_id == params.goodsid && query.amount >= params.goodsprice*0.9&&query.amount <= params.goodsprice)
        {
            ...
            request(options, function (error, response, body) {
                console.log('cpCallbackError'+JSON.stringify(error));
                console.log('cpCallbackBody'+JSON.stringify(body));
                if(!error && response.statusCode == 200){
                    var retOut = body;
                    ...
                    if(retOut.code =='0'){
                        ...
                        if(query.original_price*100>=retOut.amount*0.9
                            &&query.original_price*100<=retOut.amount){
                                if(retOut.status=='4'||retOut.status=='3'){
                                    logicCommon.UpdateOrderStatus(game,channel,retValue.cporder,retValue.order,4,query.original_price*100);
                                    retf('success');
                                    return;
                                }else{
...
function callGamePay(attrs,gattrs,params,query,ret,retf,game,channel,channelId)
{
    ...
    logicCommon.getNotifyUrl(retValue.cporder,params,function(hasData) {
        if (!hasData) {
            ...
            retValue.amount = '' + query.original_price*100 + '';
...
function checkSignPay(attrs,query)
{
    ...
    str=str+'key='+attrs.secret_key;
    console.log(str);
    //var osign = crypto.createHash('md5').update(logicCommon.utf16to8(str)).digest('hex').toLocaleLowerCase();
    var osign = crypto.createHash('md5').update(str).digest('hex').toLocaleLowerCase();
...
function checkChOrder(game, channel,attrs, query, retf){
    var isIllegal = false;
    logicCommon.selCHOrderInRedis(channel,query.out_trade_no,function(res){
        if(!res || typeof res == "undefined"){
            logicCommon.saveCHOrderInRedis(game, channel, query.cooperator_order_no, query.out_trade_no,function(res){
                if(res && typeof res != "undefined"){
                    isIllegal = true;
                    retf(isIllegal);
                }else{
