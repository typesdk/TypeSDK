...
function createSignLogin(query,attrs)
{
    return crypto.createHash('md5').update(attrs.app_id + query.token + attrs.secret_key).digest('hex');
}
...
function callChannelLogin(attrs,params,query,ret,retf,gattrs)
{
    ...
    cloned.sign = createSignLogin(cloned,attrs);
...
function checkSignPay(attrs,query)
{
    ...
    var str = 'uid=' + query.uid +
        ...
        'ext=' + query.ext+attrs.secret_key;
...
function checkChOrder(game, channel,attrs, query, retf){
    ...
    logicCommon.selCHOrderInRedis(channel,query.orderNo,function(res){
        if(!res || typeof res == "undefined"){
            logicCommon.saveCHOrderInRedis(game, channel, query.ext, query.orderNo,function(res){
                if(res && typeof res != "undefined"){
                    isIllegal = true;
                    retf(isIllegal);
                }else{
