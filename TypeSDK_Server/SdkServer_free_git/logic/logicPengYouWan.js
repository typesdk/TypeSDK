...
function convertParamLogin(query,ret)
{
    ...
    ret.token = cloned.token.toString();
    ret.uid = cloned.id.toString();
...
function callChannelLogin(attrs,params,query,ret,retf)
{
    ...
    merge(cloned,query);
    cloned.tid = new Date().getTime().toString();
...
function compareOrder(attrs,gattrs,params,query,ret,game,channel,retf){
    ...
    var retValue = {};
    retValue.code = 0;
    retValue.id =query.tid;
    retValue.order = query.ch_orderid;
    retValue.cporder = query.cp_orderid;
    retValue.info = '';
...
function checkChOrder(game, channel,attrs, query, retf){
    var isIllegal = false;
    logicCommon.selCHOrderInRedis(channel,query.cp_orderid,function(res){
        if(!res || typeof res == "undefined"){
            logicCommon.saveCHOrderInRedis(game, channel, query.ch_orderid, query.cp_orderid,function(res){
                if(res && typeof res != "undefined"){
                    isIllegal = true;
                    retf(isIllegal);
                }else{
