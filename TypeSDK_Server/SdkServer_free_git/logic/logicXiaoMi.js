...
function callChannelLogin(attrs,params,query,ret,retf)
{
    ...
    cloned.signature = createSignLogin(cloned,attrs.secret_key);
...
function compareOrder(attrs,gattrs,params,query,ret,game,channel,retf){
    var retValue = {};
    retValue.code = query.orderStatus  == 'TRADE_SUCCESS' ? 0 : 1;
                ...
                if(!error && response.statusCode == 200){
                    ...
                    if(retOut.code=='0'){
                        ...
                        if(query.uid==retOut.id
                            &&query.cpOrderId==retOut.cporder
                            &&query.payFee>=retOut.amount*0.9
...
function checkSignPay(attrs,query)
{
    ...
    var str =
        ...
        (data.cpUserInfo ? ("cpUserInfo=" + data.cpUserInfo + "&") : "") +
        (data.orderConsumeType ? ("orderConsumeType=" + data.orderConsumeType + "&") : "") +
    ...
    var osign = crypto.createHmac('sha1',attrs.secret_key).update(str).digest('hex');
...
function checkChOrder(game, channel,attrs, query, retf){
    ...
    logicCommon.selCHOrderInRedis(channel,query.orderId,function(res){
        if(!res || typeof res == "undefined"){
            logicCommon.saveCHOrderInRedis(game, channel, query.cpOrderId, query.orderId,function(res){
                if(res && typeof res != "undefined"){
                    isIllegal = true;
                    retf(isIllegal);
                }else{
