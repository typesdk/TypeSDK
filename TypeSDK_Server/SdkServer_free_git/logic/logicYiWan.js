...
function compareOrder(attrs,gattrs,params,query,ret,game,channel,retf)
{
    ...
    retValue.order = query.ordernum;
    retValue.cporder = query.custominfo || '';
    retValue.info = '';
    if(retValue.code!='0'){
        retf('FAILURE');return;
    }
...
function checkChOrder(game, channel,attrs, query, retf){
    var isIllegal = false;
    logicCommon.selCHOrderInRedis(channel,query.ordernum,function(res){
        if(!res || typeof res == "undefined"){
            logicCommon.saveCHOrderInRedis(game, channel, query.custominfo, query.ordernum,function(res){
                if(res && typeof res != "undefined"){
                    isIllegal = true;
                    retf(isIllegal);
                }else{
