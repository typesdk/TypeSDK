...
function compareOrder(attrs,gattrs,params,query,ret,game,channel,retf){
    var retValue = {};
    retValue.code = query.Pay_Status  == '1' ? 0 : 1;
    retValue.id = query.Uin;
    retValue.order = query.Recharge_Id;
    retValue.cporder = query.Urecharge_Id || '';
            ...
            request(options, function (error, response, body) {
                if(!error && response.statusCode == 200){
                    ...
                    if(retOut.code=='0'){
                        ...
                        if(query.Uin==retOut.id
                            &&query.Urecharge_Id==retOut.cporder
                            &&(query.Recharge_Money)*100>=retOut.amount*0.9
                            &&(query.Recharge_Money)*100>=retOut.amount){
                                if(retOut.status=='2'){
                                    retf('failure');
                                    return;
                                }else if(retOut.status=='4'||retOut.status=='3'){
                                    logicCommon.UpdateOrderStatus(game,channel,retValue.cporder,retValue.order,4,query.Recharge_Money*100);
                                    retf('success');
                                    return;
                                }else{
...
function callGamePay(attrs,gattrs,params,query,ret,retf,game,channel,channelId)
{
    ...
    logicCommon.getNotifyUrl(retValue.cporder,params,function(hasData) {
        if (!hasData) {
            ...
            retValue.amount = '' + (query.Recharge_Money)*100 + '';
...
function checkChOrder(game, channel,attrs, query, retf){
    var isIllegal = false;
    logicCommon.selCHOrderInRedis(channel,query.Recharge_Id,function(res){
        if(!res || typeof res == "undefined"){
            logicCommon.saveCHOrderInRedis(game, channel, query.Urecharge_Id || '', query.Recharge_Id,function(res){
                if(res && typeof res != "undefined"){
                    isIllegal = true;
                    retf(isIllegal);
                }else{
